...
class Ref(object):
    ...
    def __init__(self, node, num_refugees):
        ...
        for x in range(num_kin):
            kin = x
            while kin != x:
                kin = random.randint(0, num_refugees)
            self.kin_list[kin] = 1
            # set for other kin
        ...
        for x in range(num_friend):
            friend = x
            while friend != x:
                friend = random.randint(0, num_refugees)
...
class Sim(object):
    ...
    def __init__(self, graph, num_steps=10):
        ...
        num_refugees = sum([self.graph.nodes[n]['weight'] for n in self.graph.nodes])
        self.all_refugees = []
        for n in self.graph.nodes():
            self.all_refugees.extend([Ref(n, num_refugees) for x in range(self.graph.nodes[n]['weight'])])
        self.max_pop = None
    ...
    def sim_step(self, step):
        ...
        orig_weights = {key: self.graph.nodes[key]['weight'] for key in self.graph.nodes}
