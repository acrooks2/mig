...
config = {
    ...
    'output_dir': './test_reduced', # name of test
    ...
    'num_friends': (1,5),  # (5, 10),  # int for defined number. Tuple (low, high) for random number of friends
    'num_kin': (1,5),  # (5, 10),  # int for defined number. Tuple (low, high) for random number of friends
    ...
    'location_weight': 0.5,  # closeness to LOCATION point
    'camp_weight': 1,  # (camps * CAMP_WEIGHT)
    'conflict_weight': 00,  # (conflicts * (-1) * CONFLICT_WEIGHT)
...
class Sim(object):
    ...
    def step(self):
        ...
        if self.num_processes > 1:
            ...
            results = [self.process_refs((0, len(self.all_refugees)))]
